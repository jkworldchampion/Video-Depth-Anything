...
class DeformableCrossFrameAttention(nn.Module):
    ...
    def forward(self, query, reference, spatial_size, n_points=None):
        ...
        # determine how many sampling points to use
        if n_points is None:
        ...
        k, v = kv[:,:,0], kv[:,:,1]  # each [B, N, heads, head_dim]

        # 2) Predict offsets, keep only n_points
        offsets = self.offset_pred(query).view(
            B, N, self.num_heads, self.max_points, 2
        )[:, :, :, :n_points]  # [B, N, heads, n_points, 2]

        # 3) Build base grid (y,x) and add offsets
        ys = torch.arange(H, device=query.device)
        xs = torch.arange(W, device=query.device)
        coords = torch.stack(
            torch.meshgrid(ys, xs, indexing='ij'), -1
        ).view(1, N, 1, 1, 2).float()  # [1, N, 1, 1, 2]
        ...
        # 5) Prepare grid and reference for F.grid_sample
        #    grid: [B*heads*n_points, H, W, 2]
        grid = sample_locs.view(
            B, self.num_heads * n_points, H, W, 2
        ).reshape(
            B * self.num_heads * n_points, H, W, 2
        )
        #    ref_k_exp: [B*heads*n_points, C, H, W]
        ref_k = reference.view(B, C, H, W)
        ref_k_exp = ref_k.unsqueeze(1).expand(
            -1, self.num_heads * n_points, -1, -1, -1
        ).reshape(
            B * self.num_heads * n_points, C, H, W
        )

        # 6) Sample keys/values
        sampled = F.grid_sample(ref_k_exp, grid, align_corners=True)
        #    sampled: [B*Hn, C, H, W], where Hn=heads*n_points

        # 7) Reshape back to [B, heads, n_points, head_dim, H, W]
        sampled = sampled.view(
            B,
            self.num_heads * n_points,
            C,
            H,
            W
        ).view(
            ...
            H,
            W
        )  # [B, heads, n_points, head_dim, H, W]

        # 8) Permute to [B, H, W, heads, n_points, head_dim]
        sampled = sampled.permute(0, 4, 5, 1, 2, 3)

        # 9) Merge spatial dims: [B, N, heads, n_points, head_dim]
        ...
        # q: [B, N, heads, 1, head_dim]
        q = q.unsqueeze(3)
        # sampled_k: [B, N, heads, n_points, head_dim]
        attn = (q * sampled_k).sum(-1) * self.scale  # [B, N, heads, n_points]
        attn = attn.softmax(-1)

        # 11) Weighted sum over the n_points
